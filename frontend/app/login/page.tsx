...
import {
    ArrowLeft, Mail, Phone, Lock, Truck, Package, LayoutDashboard,
    ChevronRight, Building2, User, CheckCircle2, Grip
} from 'lucide-react';
    ...
    return (
        <div className="min-h-screen bg-slate-50 flex overflow-hidden">
            ...
            <div className="w-full lg:w-1/2 flex flex-col p-6 lg:p-12 relative z-10 bg-white overflow-y-auto">
                ...
                <Link href="/" className="inline-flex items-center text-sm text-slate-500 hover:text-slate-900 transition-colors mb-6 w-fit">
                ...
                <div className="flex-1 flex flex-col justify-center max-w-md mx-auto w-full">
                    ...
                    >
                        <div className={`w-12 h-12 rounded-xl bg-${theme.color}-50 flex items-center justify-center mb-6 ${theme.accent}`}>
                            {theme.icon}
                        </div>
                    ...
                    <div className="flex p-1 bg-slate-100 rounded-lg mb-6">
                        {['customer', 'driver', 'admin'].map((role) => (
                            <button
                                key={role}
                                onClick={() => { setActiveRole(role); setOtpSent(false); }}
                            ...
                            >
                                <div className="flex gap-4 p-1">
                                    <button
                                        ...
                                        className={`flex-1 flex items-center justify-center gap-2 py-3 border-2 rounded-xl transition-all ${userType === 'individual'
                                    ...
                                    <button
                                        ...
                                        className={`flex-1 flex items-center justify-center gap-2 py-3 border-2 rounded-xl transition-all ${userType === 'business'
                                            ? `border-${theme.color}-600 bg-${theme.color}-50 text-${theme.color}-700`
                                            : 'border-slate-100 bg-white text-slate-500 hover:border-slate-200'
                                            }`}
                                    >
                                ...
                                {!otpSent ? (
                                    ...
                                    <div className="space-y-1">
                                        <div className="flex justify-between">
                                            ...
                                            <button type="button" onClick={() => setOtpSent(false)} className={`text-xs font-bold ${theme.accent}`}>Change Number</button>
                                        </div>
                                        <div className="flex gap-2">
                                            {[1, 2, 3, 4].map((_, i) => (
                                                <input key={i} type="text" maxLength={1} className="w-full py-3 text-center bg-slate-50 border border-slate-200 rounded-xl focus:ring-2 focus:ring-slate-200 text-lg font-bold" />
                                            ))}
                                        </div>
                                    </div>
                                )}
                            ...
                            <motion.div initial={{ opacity: 0, x: 10 }} animate={{ opacity: 1, x: 0 }} className="space-y-4">
                                <div className="space-y-1">
                                    ...
                                    <div className="relative">
                                        ...
                                        <input type="email" placeholder="name@company.com" className="w-full pl-10 pr-4 py-3 bg-slate-50 border border-slate-200 rounded-xl focus:outline-none focus:ring-2 focus:ring-slate-200 transition-all" />
                                    </div>
                                </div>
                                <div className="space-y-1">
                                    ...
                                    <div className="relative">
                                        ...
                                        <input type="password" placeholder="••••••••" className="w-full pl-10 pr-4 py-3 bg-slate-50 border border-slate-200 rounded-xl focus:outline-none focus:ring-2 focus:ring-slate-200 transition-all" />
                                    </div>
                                </div>
                            </motion.div>
                        )}
                    ...
                    <p className="mt-8 text-center text-slate-500 text-sm">
                        New here? <Link href="/register" className={`font-bold hover:underline ${theme.accent}`}>Create Account</Link>
                    </p>
            ...
            <div className="hidden lg:block w-1/2 relative bg-slate-900 overflow-hidden">
                <AnimatePresence mode="wait">
                    ...
                    >
                        ...
                        <div className={`absolute inset-0 bg-gradient-to-br ${theme.bgGradient} opacity-90 mix-blend-multiply`} />
                    </motion.div>
                </AnimatePresence>
                ...
                <div className="absolute inset-0 flex flex-col justify-end p-12 text-white z-10">
                    ...
                    >
                        ...
                        <p className="text-white/70 text-lg">
                            {activeRole === 'customer' ? 'Join 50,000+ businesses and individuals moving goods securely.' : ''}
                            {activeRole === 'driver' ? 'Get paid instantly and choose your own working hours.' : ''}
                        </p>
                    </motion.div>
                </div>
            </div>
